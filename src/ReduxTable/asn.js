...
import {useFetchAllRecordsQuery} from "./recordSlice";
...
export default function Asn() {
  ...
  const records = useFetchAllRecordsQuery('asn');
  const status = records.status;
  let rows =[];

  let error = null;
  if(status === 'fulfilled'){
    rows = records.data.records.map(dataRow => {
